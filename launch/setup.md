...
- for this project, the domain is `fmc.guru`
...
```text
https://web.archive.org/web/20160328162852/fmc.guru/
```
...
```bash
railway init -n fmc.guru -w <workspace-id> --json
railway add --service fmc.guru --json
railway up --service fmc.guru --detach --verbose
railway domain --service fmc.guru --json
railway variable set SITE_URL=https://fmc.guru --service fmc.guru --environment production --skip-deploys
```

If `railway domain fmc.guru ...` returns `Unauthorized` even after `railway login`, use the Railway dashboard or Railway GraphQL API to create the custom domain. In this run, the CLI was intermittently authenticated for `status` but unauthorized for `domain`.
...
Railway gave these records for `fmc.guru`:
...
Zone > DNS > Edit
Zone > Zone > Read
Zone Resources: Include > Specific zone > fmc.guru
```
...
```bash
curl -s "https://cloudflare-dns.com/dns-query?name=fmc.guru&type=A" \
  -H "accept: application/dns-json"

curl -s "https://cloudflare-dns.com/dns-query?name=_railway-verify.fmc.guru&type=TXT" \
  -H "accept: application/dns-json"
```
